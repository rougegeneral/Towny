...
public class TownyEntityListener implements Listener {
	...
	public void onEntityDeath(EntityDeathEvent event) {
		...
		if (entity instanceof Monster) {
			...
			if (townyWorld.isUsingTowny()) {
				...
				TownBlock townBlock = townyWorld.getTownBlock(Coord.parseCoord(loc));
	...
	public void onEntityDamageByEntityEvent(EntityDamageByEntityEvent event) {
		...
		if (townyWorld == null) {
			TownyMessaging.sendErrorMsg("Could not find world");
			return;
		}
		...
		if (entity instanceof ArmorStand || entity instanceof ItemFrame || entity instanceof EnderCrystal 
			...
			if (event.getDamager() instanceof Projectile) {
				...
				if (remover instanceof Monster) {
					...
					if (defenderTB == null) {
						TownyMessaging.sendErrorMsg("Could not get defender town block" + coord);
						return;
					}
	...
	public boolean locationCanExplode(TownyWorld world, Location target) {
		...
		if (townBlock == null || townBlock.getTown() == null) {
			return world.isExpl();
		}
		
		if (world.isUsingTowny() && !world.isForceExpl()) {
			return (townBlock.getPermissions().explosion) && (!TownyAPI.getInstance().isWarTime() || !TownySettings.isAllowWarBlockGriefing() || townBlock.getTown().hasNation() || townBlock.getTown().isBANG());
		}
	...
	public void onEntityExplode(EntityExplodeEvent event) {
		...
		if (TownyAPI.getInstance().isWarTime()) {
			...
			while (it.hasNext()) {
				...
				if (townBlock == null) {
					TownyMessaging.sendErrorMsg("Town block could not be found " + block.getLocation());
					continue;
				}
				...
				if (!isNeutralTownBlock) {
					...
					if (!townyWorld.isForceExpl()) {
						...
						// If the townblock has no town it's the wilderness.
...
//		}
	...
	public void onPigHitByLightning(PigZapEvent event) {
		...
		TownyWorld townyWorld = TownyAPI.getInstance().getDataSource().getWorld(event.getEntity().getWorld().getName());
