...
public class NationType extends ChatType {
	...
	public Collection<Player> getRecipients(Collection<Player> recipients, Player player) {
		...
		try {
			final UUID nation = townyUniverse.getDataSource().getResident(player.getName()).getTown().getNation().getIdentifier();

			Collection<Player> newRecipients = new HashSet<>();

			for(Player p : recipients) {
				if(townyUniverse.getDataSource().getResident(p.getName()).getTown().getNation().getIdentifier().equals(nation)) {
					newRecipients.add(p);
				}
			}
